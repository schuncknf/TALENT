...
namespace HO {
          ...
          // this function returns the wave function of the harmonic oscillator //
...
//   double x=r/b;
// 
//   return    gsl_pow_int(x, (l +1)) * exp (- x*x/ 2.) * gsl_sf_laguerre_n((n - 1), l + 1./2., x*x);
// }
// 
// double ho_N (int n, int l, double b)
// {  n++; return  sqrt((pow(2,n+l+1) * exp((gsl_sf_lnfact(n-1)-gsl_sf_lndoublefact(2*(n-1)+2*l+1))))/ (b * sqrt(M_PI))); }
